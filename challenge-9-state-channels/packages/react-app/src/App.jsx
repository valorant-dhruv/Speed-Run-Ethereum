...
function App(props) {
  ...
  */
  ...
  async function reimburseService(wisdom) {
    ...
    //This is the cost per character
    //wisdom is the string that the GURU wishes to send
  ...
  */
  ...
  function recieveVoucher(clientAddress) {
    ...
    function processVoucher(voucher) {
      ...
      //Now we have the BigNumber representation of the updated balance

      //We again perform the same set of operations on the updated balance

      //Pack the integer value then create a hash for it and then finally arrayify the value
      const packed = ethers.utils.solidityPack(["uint256"], [updatedBalance]);
      const hashed = ethers.utils.keccak256(packed);
      const arrayified = ethers.utils.arrayify(hashed);

      //Now we will be using the ethers.utils.verifyMessage() function
      //The verifyMessage() function will return the signer of the voucher
      //The verifyMessage() function takes in two parameters as an input
      //How verifyMessage works? Lets say there is a message that the account address has signed
      //Now we have a signature of the signed message
      //Based on the signature and the message we can get back the account address that signed it

      //In our case we signed the updated balance
      //And then we stored the signature

      //From this we get the signer of the message
      let signer=ethers.utils.verifyMessage(arrayified,voucher.data.signature);


      //Now that we have got the signer of the message we need to compare whether the signer is same as the client address or not
      if(signer!=clientAddress)
      {
        console.log("The voucher has been signed by a different message sender");
        return;
      }
  ...
  function provideService(clientAddress) {
    ...
    //This means that there is some channel input from this client address
    ...
    if (channelInput) {
      ...
      //This is a side quest which we need to figure out
      //Now we need to compare the size of the best voucher with size of the provided wisdom

      //There is this
      //If the different between the best voucher and the provided wisdom has a large discrepency then reject it

      // vouchers()[clientAddress] 
      // if(wisdom.length-vouchers()[clientAddress])
      // {
